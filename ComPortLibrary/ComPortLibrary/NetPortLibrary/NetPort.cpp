...
int NetPort::read(QByteArray & rec, int ms)
{
    int len = 255;
    return read((uint8_t*)rec.data(), len, ms);
}
...
int NetPort::write(QByteArray buf, int ms)
{
    return write((uint8_t*)buf.data(), buf.length(), ms);
}
void NetPort::clear_read()
{
